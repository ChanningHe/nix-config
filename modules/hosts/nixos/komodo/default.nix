...
let
  ...
  # TOML format for structured config generation
  settingsFormat = pkgs.formats.toml { };

  # Generate structured config with proper filtering
  genFinalSettings = 
    let 
      baseSettings = {
        # Core settings
  ...
  genConfigFile = 
    ...
    then settingsFormat.generate "komodo-periphery.toml" genFinalSettings
    else cfg.configFile;
in
{
  options.services.komodo-periphery = {
    ...
    extraConfig = lib.mkOption {
      ...
      description = ''
        ...
        DEPRECATED: Use extraSettings instead for structured configuration.
      '';
      example = ''
        [secrets]
    ...
    extraSettings = lib.mkOption {
      ...
      description = ''
        ...
        This will be merged with the base settings.
      '';
      example = {
        secrets = {
          GITHUB_TOKEN = "ghp_xxxx";
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."komodo-periphery/config.toml" = lib.mkIf (cfg.configFile == null) (
      if cfg.extraConfig == ""
      then {
        # Pure structured config
        source = genConfigFile;
        mode = "0400";
        user = cfg.user;
        group = cfg.group;
      }
      else {
        # Legacy mode: merge structured config with extraConfig
        text = builtins.readFile genConfigFile + "\n" + cfg.extraConfig;
        mode = "0400";
        user = cfg.user;
        group = cfg.group;
      }
