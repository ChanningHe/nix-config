...
# Generate facter.json for the current host into hosts/nixos/$(hostname)/facter.json
facter:
    #!/usr/bin/env bash
    set -euo pipefail
    dest="hosts/nixos/$(hostname)/facter.json"
    sudo nix run --option experimental-features "nix-command flakes" nixpkgs#nixos-facter -- -o "$dest"
